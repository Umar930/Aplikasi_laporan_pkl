...
@section('title', 'Tambah Data Observasi')
...
<div class="container mt-4">
    ...
    <div class="row g-3 mb-4">
        ...
        <div class="col-md-6">
            ...
            <input type="text" name="pekerjaan_proyek" class="form-control">
        </div>
    </div>
                    ...
                    @foreach($items as $item)
                        <tr>
                            ...
                            <td>
                                <select class="form-select fw-bold text-center" name="observasi[{{ $item->id }}][ketercapaian]" id="" required>
                                    <option value="iya" {{ old("observasi.{$item->id}.ketercapaian") == 'iya' ? 'selected' : '' }}>Ya</option>
                                    <option value="tidak" {{ old("observasi.{$item->id}.ketercapaian") == 'tidak' ? 'selected' : '' }}>Tidak</option>
                                </select>
                            </td>
                            @if ($loop->first)
                            <td rowspan="{{ $item->count() }}">
                                <textarea name="observasi[{{ $item->id }}][deskripsi]" value="{{ old('observasi.{ $item->id }.deskripsi') }}" class="form-control" id=""></textarea>
                            </td>
                            @endif
                        </tr>
                        @if($loop->last)
                        <tr>
                            ...
                            <td class="text-center">
                                <input type="number" step="0.01" name="observasi[{{ $item->id }}][skor]" value="{{ old("observasi.{ $item->id }.skor") }}" class="form-control fw-bold">
                            </td>
                        </tr>
                        @endif
                        @endforeach
    ...
    <div class="d-flex justify-content-start gap-2 mt-4">
        ...
        <button type="submit" class="btn btn-primary">Input Observasi Siswa</button>
    </div>
</form>
@endif
        ...
        <form action="{{ route('dudi.observasi.store') }}" method="POST">
            @csrf
